...
{  
	u8 byte=0;   
	GPIO_ResetBits(GPIOA, GPIO_Pin_2);//SPI_FLASH_CS=0;                            //使能器件   
	SPI1_ReadWriteByte(W25X_ReadStatusReg);    //发送读取状态寄存器命令    
	byte=SPI1_ReadWriteByte(0Xff);             //读取一个字节  
	GPIO_SetBits(GPIOA, GPIO_Pin_2);//SPI_FLASH_CS=1;                            //取消片选     
	return byte;   
} 
//写SPI_FLASH状态寄存器
...
{
	u16 Temp = 0;	  
	GPIO_ResetBits(GPIOA, GPIO_Pin_2);//SPI_FLASH_CS=0;				    
	...
	GPIO_SetBits(GPIOA, GPIO_Pin_2);//SPI_FLASH_CS=1;				    
	return Temp;
}   		    
//读取SPI FLASH  
//在指定地址开始读取指定长度的数据
