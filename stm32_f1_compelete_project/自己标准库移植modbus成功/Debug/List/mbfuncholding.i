...
eMBFuncWriteHoldingRegister( UCHAR * pucFrame, USHORT * usLen )
{
    ...
    if( *usLen == ( ( 4 ) + 1 ) )
    {
        ...
        usRegAddress |= ( USHORT )( pucFrame[( 1 + 0) + 1] );
        usRegAddress++;

         
        eRegStatus = eMBRegHoldingCB( &pucFrame[( 1 + 2 )],
...
eMBFuncReadHoldingRegister( UCHAR * pucFrame, USHORT * usLen )
{
    ...
    if( *usLen == ( ( 4 ) + 1 ) )
    {
        ...
        if( ( usRegCount >= 1 ) && ( usRegCount <= ( 0x007D ) ) )
        {
...
eMBFuncReadWriteMultipleHoldingRegister( UCHAR * pucFrame, USHORT * usLen )
{
    ...
    if( *usLen >= ( ( 9 ) + 1 ) )
    {
        usRegReadAddress = ( USHORT )( pucFrame[( 1 + 0 )] << 8U );
        usRegReadAddress |= ( USHORT )( pucFrame[( 1 + 0 ) + 1] );
        usRegReadAddress++;

        usRegReadCount = ( USHORT )( pucFrame[( 1 + 2 )] << 8U );
        usRegReadCount |= ( USHORT )( pucFrame[( 1 + 2 ) + 1] );

        usRegWriteAddress = ( USHORT )( pucFrame[( 1 + 4 )] << 8U );
        usRegWriteAddress |= ( USHORT )( pucFrame[( 1 + 4 ) + 1] );
        usRegWriteAddress++;

        usRegWriteCount = ( USHORT )( pucFrame[( 1 + 6 )] << 8U );
        usRegWriteCount |= ( USHORT )( pucFrame[( 1 + 6 ) + 1] );

        ucRegWriteByteCount = pucFrame[( 1 + 8 )];
            ...
            ( ( 2 * usRegWriteCount ) == ucRegWriteByteCount ) )
        {
            ...
            eRegStatus = eMBRegHoldingCB( &pucFrame[( 1 + 9 )],
